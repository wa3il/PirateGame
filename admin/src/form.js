...
import API_EXPRESS_PATH from "./apiconfig";
...
function sendZrr() {
	...
	fetch(API_EXPRESS_PATH+'/admin/zrr', {
		method: 'POST',
...
function setTtl() {
	...
	console.log(corps);
	console.log('Token:', localStorage.getItem('token'));
	fetch(API_EXPRESS_PATH+'/admin/resources/ttl', {
		method: 'POST',
		body : JSON.stringify(corps),
		headers: {
			// autorisation
...
function getResources(mymap){
	...
	fetch(API_EXPRESS_PATH+'/api/resources', requestConfig)
		.then(response => {
